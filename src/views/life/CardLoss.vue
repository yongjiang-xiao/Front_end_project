...
const validateForm = () => {
  ...
  if (!form.studentId) {
    ...
    isValid = false;
  } else if (!/^\d{8,12}$/.test(form.studentId)) {
  ...
  if (!form.studentName) {
    ...
    isValid = false;
  } else if (!/^[\u4e00-\u9fa5]{2,10}$/.test(form.studentName)) {
    errors.studentName = '姓名格式不正确，应为2-10个中文字符';
    isValid = false;
  }
  ...
  if (!form.idCard) {
    ...
    isValid = false;
  } else if (!/^\d{17}[\dXx]$/.test(form.idCard)) {
    errors.idCard = '身份证号格式不正确';
    isValid = false;
  }

  if (form.cardNumber && !/^\d{10,16}$/.test(form.cardNumber)) {
    errors.cardNumber = '校园卡号格式不正确，应为10-16位数字';
    isValid = false;
  }
...
if (!form.lossTime) {
    ...
    const lossDate = new Date(form.lossTime);
    const oneYearAgo = new Date();
    oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);

    if (lossDate > now) {
      errors.lossTime = '丢失时间不能是未来时间';
      isValid = false;
    } else if (lossDate < oneYearAgo) {
      errors.lossTime = '丢失时间过于久远，请确认时间是否正确';
      isValid = false;
    }
  }
  return isValid;
